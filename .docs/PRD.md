...
5. **Sorting Engine (`internal/sorter/`)**: Three-file engine — `dispatcher.go` (routing + rank normalisation + fast-path checks for already-sorted, one-swap `sa`, one-rotation `ra`/`rra`), `hardcoded.go` (BFS-optimal tables for n=2..6), `chunk.go` (Butterfly sliding-window for n>6, chunk size 16 for n≤100, 32 for n>100).
...
- **Milestone 3: Base Sorting Logic** - Implementing hard-coded, BFS-optimal sorts for small sets (n=2..6) to comfortably pass the `<12 instructions` benchmark. n=7+ routes to the chunk algorithm (no strict op limit for n=6).
